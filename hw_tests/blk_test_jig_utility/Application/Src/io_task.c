...
{
	uint16_t blk_slave_1pps_dir : 1;
	uint16_t select_1pps_s0 : 1;
	uint16_t select_1pps_s1 : 1;
	uint16_t blk_master_cable_det : 1;
...
typedef struct iot_GpiPins
{
	...
	uint16_t spare_8 : 1;
	uint16_t spare_9 : 1;
	uint16_t blk_master_rack_addr : 1;
	uint16_t blk_slave_rack_addr : 1;
	uint16_t spare_12 : 1;
...
const char *lg_iot_gpi_pin_names[blk_slave_rack_addr + 1] = \
{
	"blk Master Rack Address",
	"blk Slave Rack Address"
};
...
{	/* adc_device, adc_ch_no, adc_ch_name */
	...
	{non_iso_adc, 2, "(mv) RF Mute blk Master"},
	{non_iso_adc, 3, "(mv) RF Mute blk Slave"},
	{non_iso_adc, 4, "(mv) Buzzer +12V Supply"},
	{non_iso_adc, 5, "(mA) Test Jig Current"},
	{non_iso_adc, 6, "(mv) Rem Pwr On Out blk Slave"},
	{iso_adc, 0, "(mv) RCU +12V Out"},
	{iso_adc, 1, "(mv) PoE Supply Out"},
	{iso_adc, 2, "(mv) RCU Eth Gnd"},
	{iso_adc, 3, "(mv) Prog Eth Gnd"},
	{iso_adc, 4, "(mv) blk Master Eth Gnd"},
	{iso_adc, 5, "(mv) blk Slave Eth Gnd"}
};
...
iot_GpioPinState_t iot_GetGpiPinState(iot_GpiPinId_t pin_id, const char **p_chanel_name)
{
	...
	switch(pin_id)
	{
	case blk_master_rack_addr:
		pin_state = lg_iot_gpi_pin_state.pins.blk_master_rack_addr;
		break;

	case blk_slave_rack_addr:
		pin_state = lg_iot_gpi_pin_state.pins.blk_slave_rack_addr;
		break;
...
void iot_SetGpoPinState(iot_GpoPinId_t pin_id, iot_GpioPinState_t pin_state)
{
	...
	switch(pin_id)
	{
	case blk_slave_1pps_dir:
		lg_iot_gpo_pin_state.pins.blk_slave_1pps_dir = pin_val;
		break;
	...
	case blk_master_cable_det:
		lg_iot_gpo_pin_state.pins.blk_master_cable_det = pin_val;
		break;
...
void iot_Enable1PpsOp(bool enable)
{
	if (lg_iot_initialised)
	{
		if (enable)
		{
			HAL_TIMEx_PWMN_Start_IT(lg_iot_init_data.blk_1pps_out_htim,
									lg_iot_init_data.blk_1pps_out_channel);
		}
		else
		{
			HAL_TIMEx_PWMN_Stop_IT(lg_iot_init_data.blk_1pps_out_htim,
									lg_iot_init_data.blk_1pps_out_channel);
		}
	}
}
...
bool iot_PpsDetected(uint32_t *p_pps_delta)
{
	...
	HAL_NVIC_DisableIRQ(lg_iot_init_data.blk_1pps_in_gpio_irq);
	uint32_t pps_delta = lg_iot_1pps_delta;
	uint32_t pps_previous = lg_iot_1pps_previous;
	HAL_NVIC_EnableIRQ(lg_iot_init_data.blk_1pps_in_gpio_irq);
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	...
	if (lg_iot_initialised)
	{
		if (GPIO_Pin == lg_iot_init_data.blk_1pps_in_gpio_pin)
		{
