...
static void sct_ProcessReadGpiCommand(uint8_t *resp_buf)
{
	...
	for (i = blk_master_rack_addr; i <= blk_slave_rack_addr; ++i)
	{
...
static void sct_ProcessSetGpoCommand(uint8_t *cmd_buf, uint8_t *resp_buf)
{
	...
	if (no_params == SCT_SET_GPO_CMD_FORMAT_NO)
	{
		...
		if ((gpo_pin >= blk_slave_1pps_dir) && (gpo_pin <= remote_pwr_on_in))
		{
...
/*****************************************************************************/
/**
* Sets the multiplexed source for the 1PPS input from the blk:
* 	- 0: RCU
* 	- 1: blk Master
* 	- 2: blk Slave
*
...
static void sct_ProcessSetPpsIpSrcCommand(uint8_t *cmd_buf, uint8_t *resp_buf)
{
	...
	{	/* Validate the pps_source parameter and set GPIO pin states accordingly */
		...
		case 1:
			...
			iot_SetGpoPinState(select_1pps_s1, reset);
			sprintf((char *)resp_buf, "blk Master 1PPS Source Selected%s", SCT_CRLF);
			break;

		case 2:
			iot_SetGpoPinState(select_1pps_s0, reset);
			iot_SetGpoPinState(select_1pps_s1, set);
			sprintf((char *)resp_buf, "blk Slave 1PPS Source Selected%s", SCT_CRLF);
			break;
...
/*****************************************************************************/
/**
* Sets the direction of the blk Slave 1PPS signal which is bi-directional.
*
* @param	cmd_buf command buffer to extract parameters from
* @param    resp_buf buffer for transmitting command response
* @return   None
...
static void sct_ProcessSetPpsDirCommand(uint8_t *cmd_buf, uint8_t *resp_buf)
{
	...
	{	/* If set_state is non-zero set the blk Slave as 1PPS output */
		iot_SetGpoPinState(blk_slave_1pps_dir, (direction ? set : reset));
		sprintf((char *)resp_buf, "blk Slave 1PPS direction %s%s", (direction ? "Output" : "Input"), SCT_CRLF);
	}
	else if (no_params == -1)
	{
		/* If there was an error with the number of parameters print help message */
...
/*****************************************************************************/
/**
* Enables/disables the 1PPS output to the blk
*
* @param	cmd_buf command buffer to extract parameters from
* @param    resp_buf buffer for transmitting command response
* @return   None
...
/*****************************************************************************/
/**
* Sets the multiplexed source for the UART input from the blk:
* 	- 0: blk Master
* 	- 1: blk Slave
*
* @param	cmd_buf command buffer to extract parameters from
...
static void sct_ProcessSetUartIpSrcCommand(uint8_t *cmd_buf, uint8_t *resp_buf)
{
	...
	{	/* Validate the uart_source parameter and set GPIO pin states accordingly */
		...
		case 0:
			...
			sprintf((char *)resp_buf, "blk Master UART Source Selected%s", SCT_CRLF);
			break;

		case 1:
			iot_SetGpoPinState(select_uart_s0, set);
			sprintf((char *)resp_buf, "blk Slave UART Source Selected%s", SCT_CRLF);
			break;
