...
from SPD3303C_Driver import * # required to WFH setup, remove afterwards
...
SUPPORTED_MODELS = [
    ...
    {"manufacturer": "Siglent Technologies", "model": "SPD3303C"}    # for test at WFH setup
]
...
class VisaPowerSupply:
    ...
    def device_specific_initialisation(self):
        ...
        for res in self.rm.list_resources():
            if res and (res.startswith("ASRL") or res.startswith("USB0")):
                power_supply = res
                ...
                try:
                    ...
                    # if res == "USB0::0x0483::0x7540::NPD3ECAQ1R0177::INSTR":
                        power_supply.write("*IDN?")
                        time.sleep(0.05)
                        power_supply_query_str = power_supply.read()
                        # Close the resource 
                        power_supply.close()
                    else:
                        power_supply_query_str = power_supply.query('*IDN?')
                        time.sleep(0.05) # required to use the SPD3303C on WFH setup, remove afterwards
                        # Close the resource 
                        power_supply.close()

                    if "Siglent Technologies" in power_supply_query_str:
                        # Siglent for WFH setup only
                    ...
                    elif "TENMA" not in power_supply_query_str:
                        # Not a Tenma PSU
                        self.manufacturer = power_supply_query_str.split(", ")[0]
                        self.model = power_supply_query_str.split(", ")[1]
                        self.serial_no = power_supply_query_str.split(", ")[2]
                ...
                for model in SUPPORTED_MODELS:
                    if self.model == model.get("model"):
                        ...
                        # required to use the SPD3303C on WFH setup, remove afterwards
                        elif self.model == "SPD3303C":
                            self.psu = SPD330Class()
                        try:
