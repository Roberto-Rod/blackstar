...
namespace blackstar
{
    ...
    void satpos(ephem_t eph, gpstime_t g, double *pos, double *vel, double *clk)
    {
        ...
        // RH: the below has been commented out in an attempt to make the app use out of date
        // ephemeride data. Use the true value for tk, rather than randomly subtracting vaules from
        // it. Calculations are all done using doubles, so should not cause arithmetic issues
    ...
    void usage(void)
    {
        fprintf(stderr, "Usage: gps-gen-realtime [options]\n"
            "Options:\n"
    ...
    void GPSGen::enableDebugMessages(bool enable)
    {
        m_debugMessages = enable;
    }

    void GPSGen::build(boost::lockfree::spsc_queue<iq_buff_t*, boost::lockfree::capacity<16>> *queue, int argc, char *argv[])
    {
        m_queue = queue;
        m_argc = argc;
        m_argv = argv;
    ...
    void GPSGen::start()
    {
        ...
        while ((result=getopt(m_argc,m_argv,"G:e:u:x:g:c:l:s:b:T:t:d:iv"))!=-1)
        {
            switch (result)
            {
            case 'e':
        ...
        if (g0.week>=0) // Scenario start time has been set.
        {
            ...
            else
            {
                ...
                {
                    fprintf(stderr, "WARNING: Invalid start time.\n");
                    ...
                    fprintf(stderr, "tmax = %4d/%02d/%02d,%02d:%02d:%02.0f (%d:%.0f)\n",
                        ...
                        gmax.week, gmax.sec);
                    // RH: remove the exit statement to allow to continue with stale ephemeride data
                    // exit(1);
                }
            }
        }
        ...
        // RH: force to always use the first ephemeride set from the file
        // to make sure we find ephemerides when using stale ephemeride file
        ieph = 0;
        ...
        for (iumd=1; iumd<numd && !m_done; iumd++)
        {
            for (i=0; i<MAX_CHAN; i++)
            {
                if (chan[i].prn>0)
            ...
            // Create a new IQ buffer vector and reserve its size as we know how
            // many samples we will push into it
            iq_buff_t *iq_buff = new iq_buff_t;
            iq_buff->reserve(iq_buff_size);
            ...
            for (isamp=0; isamp<iq_buff_size; isamp++)
            {
                ...
                std::complex<short> sample { (short)i_acc, (short)q_acc };
                iq_buff->push_back(sample);
            }

            //fwrite(iq_buff, 2, 2*iq_buff_size, fp);
            // Push IQ buffer (pointer to vector) onto the queue, pause if the queue is full
            while (!m_queue->push(iq_buff))
            {
                // Stop producing
                if (m_debugMessages)
                {
        ...
        if (m_done)
        {
            std::cout << "GPS generator terminated" << std::endl;
        }
        else
        {
            std::cout << "GPS generator completed" << std::endl;
        }
